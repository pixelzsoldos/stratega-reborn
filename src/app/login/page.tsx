...
export default function LoginPage() {
  ...
  const updateLogin = (k: keyof typeof loginForm, v: string) => setLoginForm(s => ({ ...s, [k]: v }))
  const updateForgot = (k: keyof typeof forgotForm, v: string) => setForgotForm(s => ({ ...s, [k]: v }))
          ...
          <form onSubmit={onLoginSubmit} noValidate>
            <div style={{ display: 'grid', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                <button type="submit" disabled={submittingLogin} style={{ padding: '8px 16px', background: palette.accent, color: palette.lightText, border: `1px solid ${palette.border}`, cursor: 'pointer' }}>Belépés</button>
              </div>
            </div>
          </form>
        </SectionCard>
        ...
        <SectionCard>
          <form onSubmit={onForgotSubmit} noValidate>
            <div style={{ display: 'grid', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                <button type="submit" disabled={submittingForgot} style={{ padding: '8px 16px', background: palette.accent, color: palette.lightText, border: `1px solid ${palette.border}`, cursor: 'pointer' }}>Mehet!</button>
              </div>
            </div>
          </form>
        </SectionCard>
